...
# Apply any overrides from addon config (synapse_config_overrides)
...
if os.path.exists(addon_path) and os.path.exists(cfg_path):
    ...
    if overrides:
        ...
        # Merge shallow (top-level keys)
        cfg.update(overrides)
        yaml.safe_dump(cfg, open(cfg_path, "w"))
PY
fi

# Ensure sqlite database initialization (matrix-synapse will create on first run)
# Optionally create admin user if requested
if [ "${ENABLE_REGISTRATION}" = "true" ] && [ -n "${REG_SECRET}" ]; then
  # ensure registration shared secret in config
